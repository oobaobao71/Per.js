...
(function(window,undefined){
    ...
    console.info("Per.js-3 Preview: The planning of Per.js-3 has been completed. It will have many new features! The first alpha version will release soon!\nPlease stay tuned!")
    ...
    window.Per.fn = function(el){
        ...
        this.dom = function(Obj,isReactive){
            ...
            if(typeof Obj == "object"&&typeof isReactive == "boolean"){
                //初始化el
                if(Obj.el != undefined){
                    console.warn("Per.js: Obj.el will remove in the next version, please change it as soon as possible!");
                }
                                            ...
                                            for(var a=0;a<dataArr.length;a++){
                                                if(a%2 == 0){
                                                   ...
                                                   if(document.querySelector(Obj.el).getAttribute("p-html") == null){
                                                    ...
                                                    if(Obj.isAsyncDOM){
                                                        joinAsyncList(document.querySelector(Obj.el),"html",html.toString());
                                                    }else{
    ...
    per.joinModule("Per.component",allVersion, function(){
        ...
        window.Per.fn.prototype.component = {
            ...
            getComponentAttr: function(componentName,attrName){//返回为数组类型
                for(var i=0,len=componentArr.length;i<len;i++){
                    if(componentArr[i] == componentName && i%2 == 0){
                        var componentElementList = document.querySelectorAll(componentName),
                            returnArray = new Array();
                        for(var a=0;a<componentElementList.length;a++){
                            returnArray[returnArray.length] = componentElementList[a].getAttribute("p-"+attrName);
                        }
                        return returnArray;
                    }
                }
    ...
    per.joinModule("Per.dom",allVersion,function(){
        ...
        window.Per.fn.prototype.dom = perGetFun;
        Per.dom = perGetFun;
        window.$ = perGetFun;
    });
    ...
    var joinAsyncHashArray = function(el,type,html){
        ...
        if(!isHasThisEl){
            asyncDOMArr[asyncDOMArr.length] = el;
            asyncDOMArr[asyncDOMArr.length] = type;
            asyncDOMArr[asyncDOMArr.length] = html;
        }
    }
})(window);